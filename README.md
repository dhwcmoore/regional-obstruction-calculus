...
- **R10**: refinement-witness composition is theorem-grade for all three governing conditions, both for sequential composition (two and three steps: `N0_composes`, `A4_composes`, `E0_composes`, `rocq/RefinementWitnessComposition.v`, `rocq/RefinementWitnessVerdictComposition.v`, `rocq/RefinementWitnessSequentialComposition.v`) and, for N0/E0 only, disjoint *parallel* composition (`N0_parallel_disjoint`, `E0_parallel_disjoint`, `rocq/RefinementWitnessParallelComposition.v`) — reached only after searches/probes (~175,000 cases sequentially, 32 cases in parallel) that also found a genuine negative result: A4 does **not** compose under disjoint parallel composition as naively stated, a demonstrated fact, not an open question.
