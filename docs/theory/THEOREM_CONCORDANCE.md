...
| Common-subdivision verdict invariance (R17) | `rocq/CommonSubdivisionVerdictInvariance.v` | `refinement_checker.py`'s `verdict_safe` field | Combines the two rows above into one theorem: for refinement legs both descent-safe (N0) and reflecting (E0), `[r_P]=0 <-> [r_Q]=0` -- proved (and, contraposed, its obstruction-side restatement) using only (N0)'s forward direction and `common_subdivision_exactness_agreement`, applied symmetrically | Not full presentation invariance (descent-safe/reflecting fragment only -- excludes `insert_bridge`, which fails N0); not class-level invariance (a correspondence between `[r_P]` and `[r_Q]` themselves); does not invoke `common_subdivision_certificate_agreement` -- see that file's own header for why (an unproved duality fact would be needed) |
...
- **No row here proves full presentation invariance.** R17 proves verdict-level invariance (`[r_P]=0 <-> [r_Q]=0`) for the descent-safe, reflecting fragment specifically -- not for arbitrary admissible refinements, and not topology-changing ones like `insert_bridge`. That restriction is deliberate; see the README's scope blockquote (levels 1, 2, 2b, 2c, 3), `docs/design/PRESENTATION_INVARIANCE_SPEC.md`, and the paper's Remark 10.8 ("Result ladder, and what remains open") for the exact boundary of what is and is not claimed.
