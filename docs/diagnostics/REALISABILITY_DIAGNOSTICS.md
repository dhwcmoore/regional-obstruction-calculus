# Realisability Diagnostics

## Question

What data must be shared for an associator residue to count as
structurally regional rather than seam-local?

## Diagnostic ladder

```text
1. Independent generator
   Result: too free.
   Image: all of C^1.
   Meaning: cannot distinguish structural residues from arbitrary choices.

2. Shared adjacent-overlap coupling
   Result: rank drop but cohomological collapse.
   Meaning: sharing exists, but only repairable residues are generated.

3. Boolean proper crossing
   Result: nontrivial obstruction witness.
   Meaning: curvature can be generated by a deterministic rule, but this
   is not a linear parameter-space result (no rank or quotient at all).

4. Ordered inclusion-exclusion
   Result: disguised independence.
   Meaning: globally indexed terms cancel and leave per-triple freedom.

5. Candidate 3b on distinct support
   Result: cover-inert.
   Meaning: no repeated support, so no real sharing -- not the same kind
   of negative result as 1 or 4; the cover cannot even test the rule.

6. Candidate 3b on repeated triple support
   Result: positive linear/rational witness.
   rank(B) = 2
   dim(im(B) ∩ im δ⁰) = 1
   dim(quotient) = 1
   Meaning: first genuine partial nontrivial quotient. Also machine-
   checked in Rocq (rocq/RepeatedTripleSupportCandidate3b.v).
```

See RESULTS.md R6-R9 for the full numbers and file references; the rest
of this document is the detailed, chronological account of how each
result was reached and verified.
