...
- **Refinement witness composition** (`refinement_witness_composition_probe.py`): does the composite of two admissible/descent-safe/exactness-reflecting refinement witnesses itself satisfy those conditions? N0-composability is provable in general (§1-adjacent — a one-line associativity argument, not yet a Rocq lemma). A4 and E0 composability are tested on two concrete composed witnesses (both positive) and not proved either way. See `docs/design/REFINEMENT_WITNESS_COMPOSITION_STATUS.md`.
