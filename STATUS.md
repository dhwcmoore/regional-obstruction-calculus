...
- **Refinement witness composition, (A4)/(E0) half** (`refinement_witness_composition_probe.py`): does the composite of two admissible/descent-safe/exactness-reflecting refinement witnesses itself satisfy (A4)/(E0)? Tested on two concrete composed witnesses (both positive) and not proved either way — (N0) composability is proved (§1), but (A4) and (E0) have no such argument yet. See `docs/design/REFINEMENT_WITNESS_COMPOSITION_STATUS.md`.
